                    ...
                    <tbody>
                        @forelse ($history as $index)
                            <tr>
                                ...
                                <img src="{{ Storage::url($index['image']) }}" class="img-fluid" style="border-radius: initial; width: 100%; height: auto; max-width: 100%;">
                            @else
                                No image available
                            @endif
                        </td>
                                ...
                                <td>
                                    <a href="{{ route('history.show', $index['ID']) }}" class="btn btn-primary btn-sm">Detail</a>
                                    <a href="{{ route('history.edit', $index['ID']) }}" class="btn btn-info btn-sm">Edit</a>
                                    <form action="{{ route('history.destroy', $index['ID']) }}" method="POST" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
